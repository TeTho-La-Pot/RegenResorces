...
public final class RegenStrippedLogResolver {
    ...
    private static Map<Block, Block> lookupStrippables() {
        for (String name : new String[]{"STRIPPABLES", "STRIPPABLES"}) {
            try {
    ...
    public static Block resolveStripped(BlockState source) {
        ...
        return RegenStrippedLogResolver.resolveStripped(source.getBlock());
    }
    ...
    public static Block fromId(@Nullable ResourceLocation id) {
        ...
        Block b = (Block)BuiltInRegistries.BLOCK.get(id);
        if (b == null) {
            return null;
        }
        ResourceLocation actual = BuiltInRegistries.BLOCK.getKey(b);
    ...
    public static ResourceLocation barkIdForStripped(@Nullable ResourceLocation strippedId) {
        ...
        for (Map.Entry<Block, Block> e : STRIPPABLES.entrySet()) {
            ...
            if (e.getValue() != stripped || (key = BuiltInRegistries.BLOCK.getKey(e.getKey())) == null) continue;
            return key;
        }
        return null;
    }
