...
public final class RegenMassBreakBugWorkaround {
    ...
    public static void onBreakSpeed(PlayerEvent.BreakSpeed event) {
        ...
        BlockPos pos = ((BlockPos)opt.get()).immutable();
        boolean sneakNow = sp.isCrouching();
        long now = System.currentTimeMillis();
        UUID id = sp.getUUID();
        Snapshot prev = SNAPSHOTS.get(id);
        if (prev == null || !prev.pos.equals(pos) || now - prev.lastSeenMs > 2000L) {
            SNAPSHOTS.put(id, new Snapshot(pos, sneakNow, now));
            return;
        }
        if (prev.sneakAtStart != sneakNow) {
            try {
                sp.gameMode.handleBlockBreakAction(pos, ServerboundPlayerActionPacket.Action.ABORT_DESTROY_BLOCK, Direction.UP, sp.serverLevel().getMaxBuildHeight(), 0);
                sp.serverLevel().destroyBlockProgress(sp.getId(), pos, -1);
                OreHarvesterChainProbe.clearCacheFor(sp.serverLevel(), sp);
            }
            catch (Throwable t) {
                LOG.debug("Failed to abort mining for {} at {}", new Object[]{sp.getName().getString(), pos, t});
            }
            SNAPSHOTS.put(id, new Snapshot(pos, sneakNow, now));
            return;
        }
