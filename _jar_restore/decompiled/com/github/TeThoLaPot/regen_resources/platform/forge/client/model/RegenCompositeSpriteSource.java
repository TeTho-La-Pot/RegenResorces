...
implements SpriteSource {
    ...
    public static final Codec<RegenCompositeSpriteSource> CODEC = Codec.unit((Object)INSTANCE);
    ...
    public static ResourceLocation sideSpriteId(ResourceLocation strippedId) {
        return ResourceLocation.fromNamespaceAndPath((String)"regen_resources", (String)(COMPOSITE_PREFIX + strippedId.m_135827_() + "_" + strippedId.m_135815_()));
    }

    public void m_260891_(ResourceManager rm, SpriteSource.Output output) {
        ...
        try (NativeImage nativeImage = mask;){
            ...
            for (Map.Entry<Block, Block> entry : pairs.entrySet()) {
                ...
                ResourceLocation barkId = BuiltInRegistries.f_256975_.m_7981_((Object)bark);
                ResourceLocation strippedId = BuiltInRegistries.f_256975_.m_7981_((Object)stripped);
    ...
    private static NativeImage readBlockSideTexture(ResourceManager rm, ResourceLocation blockId) {
        String[] swaps;
        String path;
        String ns = blockId.m_135827_();
        NativeImage img = RegenCompositeSpriteSource.readImage(rm, ResourceLocation.fromNamespaceAndPath((String)ns, (String)("textures/block/" + (path = blockId.m_135815_()) + ".png")));
        if (img != null) {
            return img;
        }
    ...
    private static boolean registerSprite(SpriteSource.Output output, ResourceLocation outRl, NativeImage composite) {
        SpriteContents contents = new SpriteContents(outRl, new FrameSize(composite.m_84982_(), composite.m_85084_()), composite, AnimationMetadataSection.f_119012_);
        output.m_260840_(outRl, (SpriteSource.SpriteSupplier)new HoldingSpriteSupplier(contents));
        return true;
    }

    private static NativeImage composite(NativeImage mask, NativeImage bark, NativeImage stripped) {
        int barkFrameH = bark.m_84982_();
        int strippedFrameH = stripped.m_84982_();
        int outW = Math.max(bark.m_84982_(), stripped.m_84982_());
        ...
        for (int y = 0; y < outH; ++y) {
            for (int x = 0; x < outW; ++x) {
                int barkArgb = RegenCompositeSpriteSource.sampleNearest(bark, x, y, outW, outH, bark.m_84982_(), barkFrameH);
                int strippedArgb = RegenCompositeSpriteSource.sampleNearest(stripped, x, y, outW, outH, stripped.m_84982_(), strippedFrameH);
                int maskArgb = RegenCompositeSpriteSource.sampleNearest(mask, x, y, outW, outH, mask.m_84982_(), mask.m_85084_());
