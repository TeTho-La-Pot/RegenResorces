...
public final class RegenMiningDelegateForgeEvents {
    ...
    private static RegenVisual resolveMiningVisual(BlockState shell) {
        if (shell.m_60734_() instanceof RegenBlocks) {
            return (RegenVisual)((Object)shell.m_61143_(RegenBlocks.VISUAL));
        }
        if (shell.m_60734_() instanceof StrippedLogPresetDummyBlock) {
            return RegenVisual.STRIPPED_LOG_PRESET;
        }
        if (shell.m_60734_() instanceof CustomPresetDummyBlock) {
            return RegenVisual.CUSTOM_PRESET;
        }
        Block block = shell.m_60734_();
    ...
    public static void onHarvestCheck(PlayerEvent.HarvestCheck event) {
        ...
        event.setCanHarvest(player.m_36298_(sample));
    }
    ...
    public static void onBreakSpeed(PlayerEvent.BreakSpeed event) {
        ...
        float shellIntr = shell.m_60800_((BlockGetter)level, pos);
        BlockState sample = RegenCorruptionFallback.miningSampleFor(vis, (BlockGetter)level, pos);
        float sampIntr = sample.m_60800_((BlockGetter)level, pos);
